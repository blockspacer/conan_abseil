...
class TestPackageConan(conan_build_helper.CMakePackage):
    ...
    def build_requirements(self):
        ...
        if self.options['abseil'].enable_tsan \
            or self.options['abseil'].enable_msan \
            or self.options['abseil'].enable_asan \
            or self.options['abseil'].enable_ubsan:
    ...
    def build(self):
        ...
        cmake.definitions['ENABLE_TSAN'] = self.options['abseil'].enable_tsan
    ...
    def test(self):
        if not tools.cross_building(self):
            ...
            bin_path = os.path.join(self.build_folder, "abseil_test_package")
            self.run("%s -s" % bin_path, run_environment=True)
            #bin_global_path = os.path.join("bin", "test_package_global")
            bin_global_path = os.path.join(self.build_folder, "abseil_test_package_global")
